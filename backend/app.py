...
app.include_router(wrd_bihar_router)  # WRD Bihar live flood stations


# ============= DATA INGESTION TARGETS =============
def get_data_ingestion_targets() -> list:
    """All stations the background pipeline should ingest periodically."""
    return [
        # Maharashtra
        IngestionTarget(state_name="Maharashtra", station_name="Kolhapur",   weather_query="Kolhapur, Maharashtra",  lat=16.705,  lon=74.243),
        IngestionTarget(state_name="Maharashtra", station_name="Nashik",     weather_query="Nashik, Maharashtra",    lat=19.997,  lon=73.791),
        IngestionTarget(state_name="Maharashtra", station_name="Pune",       weather_query="Pune, Maharashtra",      lat=18.520,  lon=73.856),
        # Bihar — WRD Bihar monitoring stations
        IngestionTarget(state_name="Bihar", station_name="Gandhi Setu",   weather_query="Patna, Bihar",       lat=25.736, lon=85.004),
        IngestionTarget(state_name="Bihar", station_name="Hajipur",       weather_query="Hajipur, Bihar",     lat=25.686, lon=85.208),
        IngestionTarget(state_name="Bihar", station_name="Dumariaghat",   weather_query="Gopalganj, Bihar",   lat=26.584, lon=86.738),
        IngestionTarget(state_name="Bihar", station_name="Basantpur",     weather_query="Gopalganj, Bihar",   lat=26.133, lon=84.367),
        IngestionTarget(state_name="Bihar", station_name="Rosera",        weather_query="Samastipur, Bihar",  lat=25.866, lon=86.011),
        # Kerala
        IngestionTarget(state_name="Kerala",       station_name="Kochi",       weather_query="Kochi, Kerala",         lat=9.931,   lon=76.267),
        # Assam
        IngestionTarget(state_name="Assam",        station_name="Guwahati",    weather_query="Guwahati, Assam",       lat=26.145,  lon=91.736),
        # Odisha
        IngestionTarget(state_name="Odisha",       station_name="Bhubaneswar", weather_query="Bhubaneswar, Odisha",   lat=20.296,  lon=85.825),
        # West Bengal
        IngestionTarget(state_name="West Bengal",  station_name="Kolkata",     weather_query="Kolkata, West Bengal",  lat=22.573,  lon=88.364),
        # Uttar Pradesh
        IngestionTarget(state_name="Uttar Pradesh",station_name="Lucknow",     weather_query="Lucknow, Uttar Pradesh",lat=26.847,  lon=80.946),
        # Gujarat
        IngestionTarget(state_name="Gujarat",      station_name="Surat",       weather_query="Surat, Gujarat",        lat=21.170,  lon=72.831),
    ]
...
def request_openweather(path: str, params: Dict[str, Any], base_url: str = "https://api.openweathermap.org") -> requests.Response:
    ...
    resp = requests.get(url, params=params, timeout=(4, 10))
    resp.raise_for_status()
    return resp


def proxy_openweather(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    cached = get_cached_weather_response(path, params)
    if cached is not None:
        return cached
    resp = request_openweather(path, params)
    data = resp.json()
