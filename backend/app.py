...
def get_source_policy_payload() -> Dict[str, Any]:
    ...
    base_sources = [
        ...
        {
            "label": "WRD Bihar",
            "title": "Bihar Flood Control Portal",
            "url": "http://fldcontrolbihar.org/",
            "usage": "Official Bihar state flood monitoring",
        },
    ]
...
def get_data_ingestion_targets() -> list[IngestionTarget]:
        ...
        for item in configured.split(","):
            ...
            if len(parts) >= 2:
                configured_targets.append(
                    IngestionTarget(state_name=parts[0], station_name=parts[1])
                )
    ...
    default_targets = [
        ...
        IngestionTarget(
            ...
            weather_query="Bhubaneswar, Odisha",
            lat=20.296,
            lon=85.824,
        ),
        IngestionTarget(
            ...
            weather_query="Kolkata, West Bengal",
            lat=22.572,
            lon=88.363,
        ),
        # --- Bihar WRD stations (live data via /api/wrd-bihar/stations) ---
        IngestionTarget(
            state_name="Bihar",
            station_name="Gandhi Setu",
            weather_query="Patna, Bihar",
            lat=25.736,
            lon=85.004,
        ),
        IngestionTarget(
            state_name="Bihar",
            station_name="Hajipur",
            weather_query="Hajipur, Bihar",
            lat=25.686,
            lon=85.208,
        ),
        IngestionTarget(
            state_name="Bihar",
            station_name="Dumariaghat",
            weather_query="Sitamarhi, Bihar",
            lat=26.804,
            lon=85.513,
        ),
    ]

    return configured_targets if configured_targets else default_targets
