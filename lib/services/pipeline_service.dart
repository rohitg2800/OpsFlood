...
class StateEntry {
  ...
  factory StateEntry.fromJson(Map<String, dynamic> j) {
    ...
    Map<String, double> _map(dynamic raw, Map<String, double> fallback) {
      if (raw is! Map) return fallback;
      return raw.map((k, v) => MapEntry(k.toString(), _d(v, 0)));
    }
    return StateEntry(
      dangerLevelM:  _d(j['danger_level_m'],  12.0),
      warningLevelM: _d(j['warning_level_m'], 10.32),
      peakLevelM:    _map(j['peak_level_m'],    {'moderate': 9.0, 'severe': 11.0, 'critical': 13.0}),
      rainfall7dMm:  _map(j['rainfall_7d_mm'], {'moderate': 200,  'severe': 400,  'critical': 650}),
    );
  }
...
class PipelineFeatures {
  ...
  factory PipelineFeatures.fromJson(Map<String, dynamic> j) {
    ...
    return PipelineFeatures(
      ...
      riverLevelM:           _d(j['river_level_m']),
      warningLevelM:         _d(j['warning_level_m']),
      dangerLevelM:          _d(j['danger_level_m']),
      rainfall1hMm:          _d(j['rainfall_1h_mm']),
      rainfall3hMm:          _d(j['rainfall_3h_mm']),
      rainfallLastHourMm:    _d(j['rainfall_last_hour_mm']),
      humidityPct:           _d(j['humidity_pct']),
      pressureHpa:           _d(j['pressure_hpa']),
      temperatureC:          _d(j['temperature_c']),
      warningHeadroomM:      _d(j['warning_headroom_m']),
      dangerHeadroomM:       _d(j['danger_headroom_m']),
      stressIndex:           _d(j['hydro_meteorological_stress_index']),
...
class PipelineService {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      // Keep using in-memory defaults — the app must not crash on cold backend
      debugLog('PipelineService.init: matrix fetch failed ($e) — using defaults');
    }
  }
  ...
  Future<void> _refreshMatrix() async {
    ...
    _matrix = raw.map((k, v) => MapEntry(k, StateEntry.fromJson(v as Map<String, dynamic>)));
  ...
  final Map<String, ({PipelineFeatures features, DateTime fetchedAt})> _featureCache = {};
  ...
  Future<PipelineFeatures?> fetchFeatures({
    ...
    final cacheKey = '${state.toLowerCase()}|${(station ?? '').toLowerCase()}';
    ...
    try {
      ...
      final res = await _client.get(uri).timeout(_timeout);
      if (res.statusCode == 404) return null; // pipeline not yet run
      if (res.statusCode != 200) throw Exception('pipeline/features HTTP ${res.statusCode}');
      ...
      _featureCache[cacheKey] = (features: features, fetchedAt: DateTime.now());
      return features;
    } catch (e) {
      debugLog('PipelineService.fetchFeatures: $e');
      return null;
    }
  }
  ...
  void debugLog(String msg) => print('[PipelineService] $msg');
}
