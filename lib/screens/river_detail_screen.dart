...
// OpsFlood — River detail screen  (v4 — abyss1 fix)
library;
...
class RiverDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: AppPalette.abyss1, // fixed: was AppPalette.abyss (doesn't exist)
      appBar: AppBar(
...
class _LevelGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    const SizedBox(width: 8),
    Expanded(child: _StatTile(label: 'Danger', value: '${data.dangerLevel.toStringAsFixed(2)} m', color: AppPalette.danger)),
  ]);
}
...
class _StatTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      color: AppPalette.abyss2, borderRadius: BorderRadius.circular(12),
...
class _RainfallCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
      _RainStat(label: 'Rainfall', value: '${data.effectiveRainfallMm.toStringAsFixed(1)} mm',
          icon: Icons.water_drop_outlined, color: const Color(0xFF38BDF8)),
      if (data.imdRainfallMm != null)
        _RainStat(label: 'IMD Rain', value: '${data.imdRainfallMm!.toStringAsFixed(1)} mm',
            icon: Icons.cloud_outlined, color: const Color(0xFF818CF8)),
      if (data.flowRate != null)
        _RainStat(label: 'Flow', value: '${data.flowRate!.toStringAsFixed(0)} m³/s',
            icon: Icons.water_outlined, color: col),
    ]),
  );
}
...
class _RainStat extends StatelessWidget {
  ...
  const _RainStat({required this.label, required this.value, required this.icon, required this.color});
  @override
...
class _RiskBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(risk, style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w800)),
  );
}
