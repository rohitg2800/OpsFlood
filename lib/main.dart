...
Future<void> main() async {
  ...
  const iosSettings = DarwinInitializationSettings(  // [change 2]
  ...
  await _localNotifications.initialize(
    ...
    onDidReceiveNotificationResponse: _onNotificationTap, // [change 2]
  );

  // ── Module 7: Notification channels & FCM topics ────────────────────────── // [change 3]
  // Creates Android importance channels (emergency/critical/warning/info)
  // and restores persisted FCM topic subscriptions.
  ...
  // ── Module 7: Alert → Notification bridge ───────────────────────────────── // [change 4]
  // Listens to AlertEngine stream; fires local push with suppression + quiet hours.
  AlertNotificationBridge.instance.start(
    DataFetchEngine.instance.alertStream,
  );

  runApp(const ProviderScope(child: FloodWatchApp()));
}

// ── Notification tap handler ────────────────────────────────────────────────── // [change 5]
// Called when the user taps a local notification (foreground or cold-start).
// `payload` is the alertId / 'test_<channel>' string from AlertNotificationBridge.
void _onNotificationTap(NotificationResponse response) {
  final payload = response.payload ?? '';
  debugPrint('[Notif] tapped: $payload');
  // Deep-link routing can be added here once a global navigator key is wired:
  // navigatorKey.currentState?.pushNamed(AlertsScreen.route);
}
    ...
    return MaterialApp(
      ...
      onGenerateRoute: (settings) {
        switch (settings.name) {
          ...
          // ── Module 6: Export Screen ────────────────────────────────────────── // [change 6a]
          case ExportScreen.route:
            return _fade(const ExportScreen());
          // ── Module 7: Notification Settings ───────────────────────────────── // [change 6b]
